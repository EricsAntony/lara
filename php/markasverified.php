...
$aid = base64_decode($_REQUEST['ass_id']);
$date = date('d/m/y');
																		...
																		<p
																			...
																			<span style="font-size:34px;"><strong><span
																						style="font-size:34px;"><span
																							style="color:#2190e3;font-size:34px;">Verification Done</span></span></strong></span>
																		</p>
																	</div>
																</div>
															</td>
																	...
																	<div class=""
																		...
																		<p
																			...
																			<span style="color:#000000;"><span
																					style="font-size:18px;">Verification
																					of your <strong>' .$sub_name. '</strong> assignment has been
																					finished. You can collect the record
																					from the lab once printing status is
																					notified. </span></span></p>
																	</div>
																</div>
															</td>
														</tr>
...
if ($val == 'not verified') {
    ...
    $mail->Subject = ('Verification of the record');
    $mail->Body = ($content);
    $mail->send();

    $sql = "UPDATE document SET v_status='verified', v_date='$date' where doc_id='$doc_id'";
    $result = mysqli_query($con, $sql);
    if ($result)
        header("refresh:0;url=../pages/assignmentpage.php?aid=".base64_encode($aid));
    else
        echo "<script>alert('failed')</script>";
} else {
    $sql = "UPDATE document SET v_status='not verified', v_date=NULL where doc_id='$doc_id'";
    $result = mysqli_query($con, $sql);
    if ($result)
        header("refresh:0;url=../pages/assignmentpage.php?aid=".base64_encode($aid));
    else
